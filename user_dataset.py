    ...
    while len(friends_data) < limit:
        ...
        if response.status_code == 200:
            ...
            for friend in friends:
                ...
                if None in (friend_name, real_name, country) or any(not s.isalnum() for s in (friend_name, real_name, country)):
                    continue 
...
for i in range(len(df_user)):
    ...
    api_url_loved_tracks = f'http://ws.audioscrobbler.com/2.0/?method=user.getlovedtracks&user={username}&api_key={API_KEY}&format=json'
